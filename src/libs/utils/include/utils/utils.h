...
inline int getLineType(char *&line, KeyWord *keywords, int nKeywords) {
    for (int i = 0; i < nKeywords; i++) {
        if (strncmp(line, keywords[i].keyWord, strlen(keywords[i].keyWord)) ==
                0 &&
            isWhiteSpace(line[strlen(keywords[i].keyWord)])) {
...
inline char *getKeyword(int lineType, KeyWord *keywords, int nKeywords) {
    for (int i = 0; i < nKeywords; i++) {
        if (lineType == keywords[i].retVal) return keywords[i].keyWord;
    }
...
inline char *trim(char *buffer) { return rTrim(lTrim(buffer)); }
