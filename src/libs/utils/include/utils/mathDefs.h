...
typedef Eigen::Matrix<double, 6, 1, Eigen::DontAlign> dVec6;
#else   // RELEASE
typedef Eigen::Matrix2d Matrix2x2;
typedef Eigen::Matrix4d Matrix4x4;
typedef Eigen::Vector2d Vector2d;
typedef Eigen::Vector4d Vector4d;
typedef Eigen::Matrix<double, 6, 1> dVec6;
#endif  // _DEBUG
...
public:
    ...
    P3D operator+(const P3D &p) { return P3D(x + p.x, y + p.y, z + p.z); }
 ...
 * possible
 */
inline P3D getP3D(const Vector3d &v) { return P3D(v.data()); }
...
public:
     ...
     */
    V3D(const Vector3d &v) { Vector3d::operator=(v); }
    ...
    V3D operator+(const V3D &v) const { return V3D(Vector3d::operator+(v)); }

    V3D operator+(const Vector3d &v) const {
        return V3D(Vector3d::operator+(v));
    }

    V3D operator-() const { return (V3D)Vector3d::operator-(); }
    ...
    V3D operator*(double val) const { return V3D(Vector3d::operator*(val)); }

    V3D operator/(double val) const { return V3D(Vector3d::operator/(val)); }

    V3D cross(const V3D &other) const { return V3D(Vector3d::cross(other)); }

    double getComponentAlong(const V3D &other) { return Vector3d::dot(other); };
...
public:
    RigidTransformation(const Quaternion &_R = Quaternion::Identity(),
                        const P3D &_T = P3D())
        : R(_R), T(_T) {}
    ~RigidTransformation() {}
