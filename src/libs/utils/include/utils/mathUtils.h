...
inline double linearlyInterpolate(double v1, double v2, double t1, double t2,
                                  double t) {
 ...
 */
inline void checkSymmetricLinearSystemResidual(const SparseMatrix &A,
                                               const dVector &x,
                                               const dVector &b) {
    // check how well we solved the linear system
    dVector rhs = A.triangularView<Eigen::Lower>() * x +
                  A.transpose().triangularView<Eigen::StrictlyUpper>() * x;
...
inline void getVectorsOrthogonalTo(const V3D &v, V3D &a, V3D &b) {
    ...
    // significant enough
    if (v[0] * v[0] / (v.squaredNorm()) <
        0.5)  // if the x component of the current vector is not too large,
...
inline V3D getRandomUnitVector() {
    return V3D(randNumberIn01Range(), randNumberIn01Range(),
               randNumberIn01Range())
        .normalized();
}
 ...
 */
inline void computeEulerAxesFromQuaternion(const Quaternion &q, V3D &a, V3D &b,
                                           V3D &c) {
...
inline void computeEulerAnglesFromQuaternion(const Quaternion &q, const V3D &a,
    ...
    Quaternion qLeft =
        getRotationQuaternion(-beta, b) * getRotationQuaternion(-gamma, c) * q;
    ...
    assert(IS_ZERO(
        (getRotationQuaternion(gamma, c) * getRotationQuaternion(beta, b) *
         getRotationQuaternion(alpha, a) * q.inverse())
            .vec()
            .norm() /
        10e5));
}

inline Matrix3x3 getRotationMatrixFromEulerAngles(const V3D &eulerAngles,
                                                  const V3D &axis_1,
                                                  const V3D &axis_2,
                                                  const V3D &axis_3) {
...
inline bool sameRotation(const Quaternion &q1, const Quaternion &q2) {
    ...
    // account...
    if (q1.isApprox(q2, 1e-7)) return true;
    Quaternion qTmp = q2;
    qTmp.w() *= -1;
    qTmp.x() *= -1;
    qTmp.y() *= -1;
...
template <typename T>
T getTargetAcceleration_explicitPD(const T& posError, const T& velError,
    double kp, double kd) {
    return -posError * kp - velError * kd;
}

template <typename T>
T getTargetAcceleration_implicitPD(const T& curPosError, const T& curVelError,
    const T& curVel, double kp, double kd,
    double dt) {
    // what we know:
    //	P_t+1 = P_t + h * V_t+1
    //	V_t+1 = V_t + h * a
    ...
    return -(curPosError * kp + curVelError * kd + curVel * dt * kp) /
        (1 + dt * dt * kp + dt * kd);
}

template <typename T>
T getTargetAcceleration_implicitPD(const T& curPos, const T& targetPos, const T& curVel, const T& targetVel, double kp, double kd,
    double dt) {
...
template <typename T>
T getTargetAcceleration_posConstraint(const T& targetPos, const T& curPos,
    const T& curVel, double dt) {
    // what we know:
    //	P_t+1 = P_t + h * V_t+1
    //	V_t+1 = V_t + h * a
...
template <class MATType>
void writeSparseMatrixDenseBlock(SparseMatrix &hes, int startX, int startY,
                                 const MATType &block,
...
template <class MATType>
void writeSparseMatrixDenseBlockAdd(SparseMatrix &hes, int startX, int startY,
                                    const MATType &block,
                                    bool writeOnlyLowerDiagonalValues = false) {
    for (int i = 0; i < block.rows(); i++)
        for (int j = 0; j < block.cols(); j++)
...
template <class MATType>
void addSparseMatrixDenseBlockToTriplet(
    std::vector<MTriplet> &triplets, int startX, int startY,
    const MATType &block, bool writeOnlyLowerDiagonalValues = false) {
    for (int i = 0; i < block.rows(); i++)
        for (int j = 0; j < block.cols(); j++)
            if (startX + i >= startY + j || !writeOnlyLowerDiagonalValues)
                triplets.push_back(
                    MTriplet(startX + i, startY + j, block(i, j)));
}

template <class MATType>
void addSparseMatrixDenseBlockToTripletAtIndex(
    std::vector<MTriplet> &triplets, int &pos_idx_io, int startX, int startY,
    const MATType &block, bool writeOnlyLowerDiagonalValues = false) {
    // int i_entry = 0;
    for (int i = 0; i < block.rows(); i++)
        for (int j = 0; j < block.cols(); j++)
...
inline void addMTripletToList_ignoreUpperElements(
    std::vector<MTriplet> &triplets, int row, int col, double val) {
    if (row >= col) triplets.push_back(MTriplet(row, col, val));
}
...
inline void addMTripletToList_mirror(std::vector<MTriplet> &triplets, int row,
                                     int col, double val) {
    if (row == col) {
        triplets.push_back(MTriplet(row, col, val));
    } else {
...
inline void addMTripletToList(std::vector<MTriplet> &triplets, int row, int col,
                              double val) {
    triplets.push_back(MTriplet(row, col, val));
}
...
inline void getOrthogonalVectors(const V3D &vec, V3D &a, V3D &b) {
    // try to choose a vector in the y-z plane, if the z-coordinate is
    // significant enough
    if (vec[0] * vec[0] / (vec.squaredNorm()) <
        0.5)  // if the x component of the current vector is not too large,
              // relatively speaking, then choose x as the main component for a
              // (they won't be aligned perfectly)
        a = V3D(1, 0, 0);
