...
public:
    ...
    // + by + cz + d = 0
    void getCartesianEquationCoefficients(double &a, double &b, double &c,
                                          double &d) {
        a = n[0];
        b = n[1];
        c = n[2];
...
public:
    ...
    P3D getPointAt(double t) const { return origin + dir * t; }
    ...
    double getDistanceToSegment(const P3D &p1, const P3D &p2,
                                P3D *closestPtOnRay) const {
    ...
    double getDistanceToPoint(const P3D &p, P3D *closestPtOnRay) const {
        ...
        P3D res = origin;
        if (d > 0) res = getPointAt(d);

        if (closestPtOnRay) *closestPtOnRay = res;
    ...
    double getDistanceToPlane(const Plane &plane, P3D *closestPtOnRay) const {
        ...
        if (fabs(dir.dot(plane.n)) < 10e-10) {
            if (closestPtOnRay) *closestPtOnRay = origin;
        ...
        if (closestPtOnRay) *closestPtOnRay = origin + dir * t;
